...
// gallery video 
	...
	public function deleteVgallery($vg_id)
    {
		
		$this->db->delete('vgallery', array('vg_id' => $vg_id)); 
    }

	public function addVgallery()
	{
		$vg_name = $this->input->post('vg_name');
		$vg_url = $this->input->post('vg_url');

				$data = array(
					'vg_name'=>$vg_name,
					'vg_url'=>$vg_url
				);
		$this->db->insert('vgallery',$data);
		
	
		redirect('Admin/videogallery');
	}
